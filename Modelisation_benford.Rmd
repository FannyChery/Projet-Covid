...
Nous obtenons la mantisse d'un réel $x$ strictement positif en déplaçant la virgule après le premier chiffre significatif. Donc la mantisse d'un nombre appartient à l'intervalle $[0,10[$ et est obtenue à partie de la formule $mantisse(x)=10^{log_{10}}$ (rappel : $\{.\}$ désigne la partie fractionnaire du nombre $x$).
...
Tout ceci se généralise bien sûr en base quelconque. Nous choisissons, communément la base $c=10$ comme base de référence pour le logarithme. Nous avons choisit la base $10$ pour le logarithme dans la suite.
...
### II.3.2 - Loi de Benford sur les deux premiers chiffres significatif
...
\includegraphics[width = 12cm]{benlaw2.png}
...
On appelle les [hypothèses de type 19](https://www.universalis.fr/encyclopedie/tests-d-hypotheses-statistiques/8-tests-d-ajustement/#) un problème de la forme suivante :
...
### III.2.2 - Test de Kolmogorov-Smirnov
...
### III.2.3 - Test du [$T_k$](https://www.openscience.fr/IMG/pdf/iste_mas20v3n1_1.pdf)
...
Après analyse nous obtenons alors la table suivante représentative des individus pour lesquels on rejette $H_0$ (voir tableau complet figure ( [$\ref{fig:2dgt}$]{.ul}) en annexe):
...
La loi de Benford pour les deux premiers chiffres significatifs s'exprime sous la forme suivante :

$$
P(I_1I_2)=log_{10}(1+1/I_1I_1)
$$

avec $I_1I_2$ représentant les deux premiers chiffres significatifs et $I_1I_2~\in~\{10,\dots,99\}$.

La loi de Benford pour les deux premiers chiffres significatifs se modélise donc de la façon suivante :
...
Après cette nouvelle étude, nous retrouvons de nouveau 26 pays suspects présentant des données douteuses que nous pouvons observer dans le tableau ci-dessous.
...
Cependant nous remarquons que certains pays ont disparu de la liste :
