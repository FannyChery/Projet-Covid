...
barres <-barplot(benlaw(digits), names.arg = digits, xlab = "First Digit",ylab="frequency", ylim = c(0, .35),col="blue")
text(x=barres[,1], y=benlaw(digits)*1.105,labels = x)
```

```{r}
library(BenfordTests)
twodigits <- function(df){
  x<-vec(df)
